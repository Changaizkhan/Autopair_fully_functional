...
def handle_question_submission(lead, question):
    ...
    # 🔒 Force fallback on unknown or non-warranty questions
    fallback_keywords = [
        "idk", "i don't know", "not sure", "maybe", "confused",
        "no idea", "help me", "can't decide",
        "cat", "dog", "pet", "alien", "spaceship", "bike", "bicycle",
        "ghost", "uncle", "random", "lost", "talk to someone"
    ]
    force_fallback = any(phrase in lower_question for phrase in fallback_keywords)

    ai_response = get_ai_response(question, context)

    # 🧠 If fallback is forced or AI gives an unsure response
    if (
        force_fallback
        or not ai_response
        or any(term in ai_response.lower() for term in [
            "trouble", "not sure", "can't answer", "unsure",
            "not confident", "you should talk to"
        ])
    ):
        ai_response = (
